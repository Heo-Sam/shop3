					...
					<tr>
						...
						<td>
							<form method="post">
								...
								<input type="hidden" name="id" value="${k.userid}">
								<input type="hidden" name="p_num" value="${k.p_num}">
								<input type="button" value="수정" onclick="edit_cart(this.form)">
							</form>
						</td>
						<td><fmt:formatNumber value="${k.p_saleprice * k.quant}" pattern="#,###" /> 원</td>
						<td>
							<form method="post">
								<input type="hidden" name="id" value="${k.userid}">
						...
						<c:set var="total" value="${total = total + (k.p_saleprice * k.quant) }"></c:set>
					</tr>	
					</c:forEach>
				</c:otherwise>
			</c:choose>
